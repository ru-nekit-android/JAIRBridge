package ru.nekit.ane 
{
	...
	public class P2PConnectionEntry
	{
		...
		private static var groupSuffix:String;
		...
		private static function connect(value:String):void
		{
			groupSuffix = value;
			if( groupSuffix == null )
			{
		...
		{
			P2PConnectionEntry.connect(value);
		}
		
		private static function groupConnectedHandler(event:GroupEvent):void
		{
			context.dispatchP2PStatusEvent(CONNECT);
			trace("P2P client connect to group");
		}
		
		private static function clientUpdateHandler(event:ClientEvent):void
		{
			trace("P2P client update: " + event.client.clientName);
		}
		
		private static function clientRemoveHandler(event:ClientEvent):void
		{
			trace("P2P client removed: " + event.client.clientName);
		}
		
		private static function groupClosedHandler(event:GroupEvent):void
		{
			trace("P2P group cloused...");
		}
		
		private static function clientAddedHandler(event:ClientEvent):void
		{
